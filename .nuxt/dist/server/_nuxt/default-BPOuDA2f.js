import { defineComponent, computed, mergeProps, unref, useSSRContext, createVNode, resolveDynamicComponent, withCtx, toDisplayString, ref, readonly } from "vue";
import { ssrRenderAttrs, ssrRenderClass, ssrInterpolate, ssrRenderVNode, ssrRenderComponent, ssrRenderList, ssrRenderAttr, ssrRenderSlot } from "vue/server-renderer";
import { u as useSmoothScroll, _ as _sfc_main$5 } from "./useSmoothScroll-D9F-S2zI.js";
import { u as useI18n, g as useSwitchLocalePath, a as useRouter } from "../server.mjs";
...
const _sfc_main$4 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<header${ssrRenderAttrs(mergeProps({ class: "fixed top-0 left-0 w-full z-40 p-5 md:p-8 pointer-events-none flex justify-between items-start glass-nav" }, _attrs))}><a href="#top" class="nav-trigger text-fluid-2xl font-display font-bold tracking-tighter text-brand-text pointer-events-auto hover:opacity-70 transition-opacity"> Mindcoded by Mo. </a><div class="flex items-center gap-3 pointer-events-auto"><div class="bg-brand-surface border border-gray-200/60 rounded-full p-1 flex items-center shadow-sm"><button class="${ssrRenderClass([unref(isEnglish) ? "bg-brand-text text-white" : "text-brand-muted hover:text-brand-text", "min-w-[44px] min-h-[44px] px-4 py-2 rounded-full text-fluid-sm font-semibold transition-all duration-300 flex items-center justify-center"])}"> EN </button><button class="${ssrRenderClass([unref(isGerman) ? "bg-brand-text text-white" : "text-brand-muted hover:text-brand-text", "min-w-[44px] min-h-[44px] px-4 py-2 rounded-full text-fluid-sm font-semibold transition-all duration-300 flex items-center justify-center"])}"> DE </button></div><a href="#contact" class="nav-trigger bg-brand-surface text-brand-text border border-gray-200/60 px-4 py-2 md:px-5 md:py-2.5 rounded-full text-fluid-sm font-semibold hover:border-brand-terra hover:text-brand-terra transition-all duration-300 shadow-sm flex items-center gap-2"><span class="relative flex h-2 w-2 md:h-2.5 md:w-2.5"><span class="animate-ping-slow absolute inline-flex h-full w-full rounded-full bg-brand-terra opacity-75"></span><span class="relative inline-flex rounded-full h-2 w-2 md:h-2.5 md:w-2.5 bg-brand-terra"></span></span> ${ssrInterpolate(_ctx.$t("nav.available"))}</a></div></header>`);
    };
  }
});
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    ref(null);
    const NAV_ITEMS = [
      { id: "top", label: "nav.home", icon: "House" },
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(NAV_ITEMS, (item) => {
        _push(`<a${ssrRenderAttr("href", `#${item.id}`)} class="${ssrRenderClass([{ active: unref(activeSection) === item.id }, "nav-item group flex items-center justify-center w-10 h-10 md:w-12 md:h-12 rounded-full text-brand-muted hover:text-brand-text hover:bg-white/60"])}"${ssrRenderAttr("aria-label", _ctx.$t(item.label))}>`);
...
//# sourceMappingURL=default-BPOuDA2f.js.map
